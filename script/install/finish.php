...
function incrementPageViews() {
	...
	$rows = mysql_num_rows(mysqlQuery("SELECT `pageViews` FROM `stats` WHERE `datetime`=CURDATE()"));
	
	if ($rows > 0) {
	
		mysqlQuery("UPDATE `stats` SET `pageViews`=`pageViews`+1 WHERE `datetime`=CURDATE()");
	
	} else {
	
		mysqlQuery("INSERT INTO `stats`(`pageViews`, `uniqueHits`, `datetime`, `clicks`) VALUES ('1','0',CURDATE(),'0')");
...
function incrementUniqueHit() {
	...
	$rows = $rows = mysql_num_rows(mysqlQuery("SELECT `pageViews` FROM `stats` WHERE `datetime`=CURDATE()"));

	if ($rows > 0) {
		
		mysqlQuery("UPDATE `stats` SET `uniqueHits`=`uniqueHits`+1 WHERE `datetime`=CURDATE()");

	} else {

		mysqlQuery("INSERT INTO `stats`(`pageViews`, `uniqueHits`, `datetime`, `clicks`) VALUES ('0','1',CURDATE(),'0')");
